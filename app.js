...
// Глобально блокируем дефолтное поведение, чтобы дроп не открывал вкладки
...
if (!JSZIP_OK) libStatus.textContent = "JSZip not found. Put real libs/jszip.min.js";
...
// восстановить прошлые настройки
(async()=>{
...
function renderCompressList() {
  ...
  $$(".item button[data-i]", listC).forEach(btn =>
    on(btn,"click",()=>{ const idx=Number(btn.getAttribute("data-i"));
...
// выбор из ФС — подменяем текущий набор (не копим мусор)
on(inC,"change",()=>{
  const chosen = Array.from(inC.files||[]);
  if (chosen.length) { files = chosen; renderCompressList(); }
  inC.value = ""; // иначе повторный выбор того же файла не триггерится
});
...
on(dzC,"drop",async(e)=>{
  ...
  // 1) Файлы из ОС
  if (dt?.files?.length) { files = Array.from(dt.files); renderCompressList(); return; }
  // 2) URL со страницы (если перетащили ссылку/картинку)
  const url = dt?.getData("URL") || dt?.getData("text/uri-list") || dt?.getData("text/plain");
  if (url) await tryAddUrlAsFile(url);
});

// загрузка файла по URL через SW (нужен host_permissions: <all_urls>)
...
async function tryAddUrlAsFile(url){
  ...
  }catch(e){ console.warn("FETCH_URL error:", e); alert("Could not fetch dropped URL."); }
}
...
on(btnZip,"click",async()=>{
  ...
  try{
    ...
    chrome.runtime.sendMessage({type:"download",payload:{url,filename:name}},(res)=>{
      ...
      progC.textContent = res?.ok ? "Saved to Downloads." : (res?.lastError || "Saved.");
    });
  }catch(e){ console.error(e); alert("Compression failed: "+e.message); }
  ...
  try{
    ...
    listV.innerHTML = entries.map(p=>{
      ...
      return `<div class="item file" data-path="${p}">
                <div class="name" title="${p}">${label}</div>
...
on(btnExtract,"click",async()=>{
  ...
  for (const [name, entry] of entries) {
    ...
    try{
      ...
      const url  = URL.createObjectURL(blob);
      const safe = name.split("\\").join("/"); // нормализация путей
      chrome.runtime.sendMessage(
        {type:"download",payload:{url,filename:`${base}/${safe}`}},
        () => setTimeout(()=>URL.revokeObjectURL(url),3000)
      );
      count++;
    }catch(e){ console.warn("Extract fail:", name, e); }
  }
...
on(fReg,"submit",async(e)=>{
  ...
  const until=Date.now()+1000*60*60*24*365*5; // +5 лет
...
on(fLogin,"submit",async(e)=>{
  ...
  const users=await getUsers(); const u=users[email];
...
// ---------- мягкая подсказка про URL DnD (если прав нет) ----------
(async ()=>{
  try {
